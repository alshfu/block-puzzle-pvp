...
## 📖 Книга-учебник «Марьям в Мире Кода» — синхронизация снимка

Книга (`book/`) учит по **настоящему коду игры** и ссылается на файлы/номера строк
(`lib/core/…`, `server/…`). Читатель клонирует не плавающий `main`, а **фикс-тег
`book-v1`** (`git checkout book-v1`). Значит, при релизе, который меняет код,
на который ссылается книга, тег и сборку книги нужно двигать — иначе номера строк
в книге разъедутся с экраном ученика.

- [ ] **Изменился код, на который ссылается книга?** (файлы/строки в `lib/core`,
  `server`, задания дней). Если нет — пропустить всё ниже.
- [ ] При необходимости обновить тексты/номера строк в источнике книги
  (`tools/book_days/weekNN.py`, `book/maryam_v_mire_koda.html`).
- [ ] Пересобрать книгу: `python3 tools/gen_book_app.py` (соберёт `web/` + JSON).
- [ ] **Передвинуть тег на новый снимок и запушить:**
  ```bash
  git tag -f -a book-v1 -m "Снимок проекта под книгу v1 (релиз <версия>)"
  git push origin -f book-v1
  ```
  (Для нового издания книги — заводи новый тег `book-v2` и меняй `git checkout`
  в тексте setup-трека, а не перезаписывай старый.)
- [ ] Проверить, что `git checkout book-v1 && flutter run -d chrome` собирается
  и номера строк из книги совпадают с файлами.
