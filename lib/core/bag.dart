...
class Bag {
  ...
  /// Подсматривает тип следующей фигуры в очереди БЕЗ выдачи (для превью
  /// «следующих фигур»). При пустой очереди дозаполняет мешок — детерминированно
  /// (тот же shuffle, что и при следующем draw), поэтому реальную выдачу не
  /// меняет.
  PieceType peek() {
    if (_queue.isEmpty) {
      _queue.addAll(_shuffle(allTypes, _rng.next));
