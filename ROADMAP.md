...
## Фаза 10 — Монетизация v2 (IAP + кастомизация) ✅ (локально, 2026-06-19; платежи/синк 🔒)

Локально-проверяемые части закрыты; реальная оплата, серверная валидация чека,
анти-fraud, маркетплейс с модерацией, онлайн-трансляция скина и push — серверная
надстройка (🔒).

### 10.1. In-app purchase 🟨 (каталог локально; оплата 🔒)
- [x] Пакеты 100 / 500 / 1500 / 5000 кристаллов с бонусом за объём —
      `lib/shop/crystal_packs.dart` (pure + тесты), вкладка «💎 Кристаллы» в
      магазине.
- [x] Граница оплаты — `lib/shop/purchase_service.dart` (`PurchaseService` +
      честный стаб «оплата не подключена»); начисление кристаллов при успехе.
- [ ] 🔒 Интеграция RuStore/App Store/Play/Stripe + server-side receipt
      validation + анти-fraud (лимиты/fingerprint/refund) — серверная надстройка.

### 10.2. Конструктор тем ✅
- [x] Премиум-инструмент (разблокировка за 5000 💎) — гейт в Настройках.
- [x] Выбор палитры (фон/градиент/панель/текст/цвета игроков/акцент) + базовый
      пресет (шрифт/радиусы), **live-preview** — `theme_builder_screen.dart`.
- [x] Модель `custom_theme.dart` (build `BlockDuelTheme` из базы + overrides,
      JSON), контроллер/стор `custom_themes_controller.dart`, активная тема
      через `activeThemeProvider` (вписана в `app.dart`); кастомные темы — в
      переключателе тем. Хранение локальное (SharedPreferences).
- [ ] 🔒 Синк `users/{uid}/customThemes` + публикация в маркетплейс с модерацией.

### 10.3. Трансляция скинов соперника 🟨 (accessibility локально; broadcast 🔒)
- [x] Accessibility-режим «нейтральные клетки» (`accessibilityNeutralSkins` в
      настройках): доска игнорирует косметический скин, рисует нейтрально.
- [ ] 🔒 `OnlineGameState.players[i].skinId` по проводу + рендер скина соперника
      + индикатор пака — требует серверной правки wire-протокола (Node/VPS).

### 10.4. Сезонный pass ✅ (локально; серверная валидация 🔒)
- [x] 90-дневный сезон (сброс по ключу дня), 50 уровней, премиум-трек за 990 💎.
- [x] Free + premium reward-треки, XP-прогресс через матчи (хук в `game_screen`).
- [x] `lib/progression/season_pass.dart` (pure + тесты) + контроллер + экран
      `season_pass_screen.dart` (маршрут `/season`, иконка 🎟 в меню).
- [ ] 🔒 Серверная валидация прогресса пропуска.

### 10.5. Расширенный магазин 🟨 (категории локально; offers/wishlist/push 🔒)
- [x] Категории-вкладки магазина: power-ups / скины / **кристаллы**
      (+ конструктор тем как вход из Настроек). Иконки — Material (см. § ASSETS).
- [ ] 🔒 Bundles, limited-time offers (24–48ч), wishlist + push при скидке
      (push — Фаза 11; rotation/скидки удобнее с серверным каталогом).
