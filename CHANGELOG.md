...
- **Панель управления фигурой** (`lib/ui/widgets/piece_controls.dart`): в
  ...
  добавлен `deselect()`.
- **Адаптивная раскладка Co-op**: на широких экранах (≥720px) высокое поле
  10×20 уходит влево, а «обвязка» (табло + рука + панель управления) —
  вертикальной панелью справа (ROADMAP § 5.4 «обвязка по бокам»); на узких —
  прежняя вертикаль. `LayoutBuilder` + `_WideLayout`/`_NarrowLayout`.
