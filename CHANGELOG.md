...
- **Панель управления фигурой** (`lib/ui/widgets/piece_controls.dart`): в
  ...
  добавлен `deselect()`.
- **Адаптивная раскладка во всех режимах с доской** (Co-op, Match-3, Memory
  Solo, Memory Duel): на широких экранах (≥720px) поле уходит влево, а «обвязка»
  (табло/таймер/рука/панель управления/подсказка) — вертикальной панелью справа
  (ROADMAP § 5.4 «обвязка по бокам»); на узких — прежняя вертикаль.
  `LayoutBuilder` + `_WideLayout`/`_NarrowLayout` (Co-op/Match-3) и ветвление
  `wide` в игровых фазах Memory-режимов.
