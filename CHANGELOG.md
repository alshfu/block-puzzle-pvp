...
- **Панель управления фигурой** (`lib/ui/widgets/piece_controls.dart`): в
  ...
  добавлен `deselect()`.
- **Адаптивная раскладка Co-op и Match-3**: на широких экранах (≥720px) поле
  уходит влево, а «обвязка» — вертикальной панелью справа (Co-op: табло + рука +
  панель управления; Match-3: табло + строка хода + подсказка), ROADMAP § 5.4
  «обвязка по бокам»; на узких — прежняя вертикаль. `LayoutBuilder` +
  `_WideLayout`/`_NarrowLayout` в обоих экранах.
