...
> 2. **Frontend** — прод Pages пока отдаёт **legacy TS-сборку** (`npm run deploy`);
>    переключение на **Flutter Web** — в разделе **«Frontend cut-over»** ниже
>    (`npm run deploy:flutter`). До cut-over актуален TS-путь, после — Flutter.
